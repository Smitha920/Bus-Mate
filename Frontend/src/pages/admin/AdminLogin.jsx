...
export default function AdminLogin() {
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #0a0e1a 0%, #0d0a1a 50%, #0a0e1a 100%)', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: '420px', background: 'rgba(17,24,39,0.95)', border: '1px solid #1e2d45', borderRadius: '24px', padding: '40px 32px' }}>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <input
              ...
              onChange={(e) => setUsername(e.target.value)}
              placeholder="admin"
              style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', background: 'rgba(255,255,255,0.04)', border: '1px solid #1e2d45', color: '#fff', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }}
              onFocus={e => e.target.style.borderColor = '#a855f7'}
              onBlur={e => e.target.style.borderColor = '#1e2d45'}
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width: '100%', padding: '16px', borderRadius: '14px', background: loading ? 'rgba(168,85,247,0.4)' : 'linear-gradient(135deg, #a855f7, #7c3aed)', border: 'none', color: '#fff', fontSize: '15px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif', boxShadow: loading ? 'none' : '0 0 30px rgba(168,85,247,0.3)' }}
          >
            {loading ? 'Logging in...' : 'Login as Admin →'}
          </button>
        ...
        <div style={{ marginTop: '24px', padding: '14px 16px', borderRadius: '14px', background: 'rgba(168,85,247,0.05)', border: '1px solid rgba(168,85,247,0.15)' }}>
          <p style={{ fontSize: '12px', color: '#6b7a99', margin: 0, textAlign: 'center', lineHeight: 1.6 }}>
            🔒 Restricted to college administrators only
          </p>
        </div>

      </div>

      <p style={{ position: 'relative', zIndex: 10, fontSize: '12px', color: '#6b7a99', marginTop: '20px', cursor: 'pointer' }}
        onClick={() => navigate('/')}>
        ← Back to Home
      </p>
